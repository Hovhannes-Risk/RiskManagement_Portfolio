...
└── sample_output/         Example PDFs from full-dataset runs


## Run locally

Both reports read the shared synthetic sample at
`../Project_A_Dashboard/sample_data/mydata_sample.xlsx` and write PDFs to a
local `outputs/` folder (gitignored). The public sample covers a single
trading day, so the weekly report's "week" collapses to one date — in
production it consumes a 7-day query export with the same schema. Committed
example PDFs from the full dataset are in `sample_output/`.
...
Both scripts write a date-stamped PDF into outputs/. Inputs are the generated analysis files; the dataset is fully synthetic (methodology.md §1).
...
- Same data provenance as Projects A and B — see Documentation/methodology.md §7
