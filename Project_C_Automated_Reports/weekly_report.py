...
# Shared anonymized sample. NOTE: the public sample covers a single trading
# day, so the "week" collapses to one date — in production this consumes a
# 7-day query export with the same schema.
DATA_PATH = SCRIPT_DIR.parent / "Project_A_Dashboard" / "sample_data" / "mydata_sample.xlsx"
...
if "bet_date" not in df.columns:
    df["bet_date"] = df["bet_time"].dt.normalize()
...
# Bot detection
df['timestamp'] = df['bet_time'].dt.floor('s')
...
pdf_path = OUT_DIR / f"weekly_executive_summary_{year}_W{week_num}.pdf"
