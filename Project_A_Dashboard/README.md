...
2. **Player drill-down** — search any anonymized ID, see their full
...
→ MINIMAL), search a specific anonymized ID, export the filtered
...
The dashboard reads the master mapping
(`bettor_mapping_PRIVATE.xlsx`) once at load. Every bettor surfaced
anywhere in the UI — KPIs, tables, drill-down search, CSV export — is
the anonymized `P{N}` form. The mapping never appears in the UI and
never leaves disk.

This is the same operational rule as the PDF reports and the analysis
pipeline: anonymize **once** upstream, every downstream surface stays
consistent. See [`Documentation/methodology.md`](../Documentation/methodology.md) §7.
...
├── sample_data/               Anonymized inputs (Project B outputs)
│   ├── mydata_sample.xlsx
│   ├── player_risk_scores.xlsx
│   ├── multi_account_detection.xlsx
│   └── bettor_mapping_PRIVATE.xlsx   *not committed*
...
- **Sample data only.** The committed `sample_data/` is anonymized
