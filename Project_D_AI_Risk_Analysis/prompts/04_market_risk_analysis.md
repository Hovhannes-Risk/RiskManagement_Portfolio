...
5. Champions League Anomaly — Statistical Read
...
3. Loss Centres — by League
   ...
   - Pay particular attention to Champions League if it appears —
...
6. Risk Level + Confidence Level
   ...
   - Champions League: depends on CI test result above.

7. Recommended Actions (trader-side, not account-side)
   - For loss-centre markets: margin review, line management,
     liability caps — NOT account restrictions.
   - For Champions League: manual settlement audit if actionable,
...
The Champions League case is the exception: a sustained 65%+ win rate
across 469 settled bets is unlikely to be a margin problem and
...
| Failure | Mitigation |
|---|---|
| LLM recommends "ban Champions League players" | Rule: "Recommended Actions (trader-side, not account-side)" |
| LLM treats Champions League and Football identically as "loss centres" | Section 5 separates Champions League into a statistical read |
...
- **v2** — Separated Champions League into its own section with the CI test.
